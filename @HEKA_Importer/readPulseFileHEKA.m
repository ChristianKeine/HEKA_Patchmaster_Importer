...
for k=1:2 %         = 152; (* ARRAY[0..1] OF UserParamDescrType = 2*40 *)
...
s.SeOldLockInParams=getSeLockInParams(fh);%       = 376; (* SeLockInSize = 96, see "Pulsed.de" *)
s.SeOldAmplifierState=getAmplifierState(fh);%     = 472; (* AmplifierStateSize = 400 *)
...
% s.SeOldLockInParams = fread(fh,96); %    = 376; (* SeOldLockInSize = 96, see "Pulsed.de" *)
% s.SeOldAmpState = fread(fh,400); %        = 472; (* SeOldAmpState = 400 -> the AmplStateRecord is now stored in the .amp file *)
